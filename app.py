...
def douglas_peucker(points, tolerance):
    ...
    dmax = 0; index = 0; end = len(points) - 1
...
def apply_dogbone_single(polygon: Polygon, tool_dia: float) -> Polygon:
    ...
    rings = [poly.exterior] + list(poly.interiors); dogbone_circles = []; r = tool_dia / 2.0
    for ring in rings:
        coords = list(ring.coords)
        if coords[0] == coords[-1]: coords.pop()
        num_pts = len(coords)
        for i in range(num_pts):
            p_curr = np.array(coords[i]); p_prev = np.array(coords[(i-1)%num_pts]); p_next = np.array(coords[(i+1)%num_pts])
            v1 = p_curr - p_prev; v2 = p_next - p_curr; n1 = np.linalg.norm(v1); n2 = np.linalg.norm(v2)
            if n1 < 1e-6 or n2 < 1e-6: continue
            v1 /= n1; v2 /= n2; cross = np.cross(v1, v2); dot = np.dot(v1, v2); angle_deg = math.degrees(math.atan2(cross, dot))
            if 5 < abs(angle_deg) < 175:
                bisector = v2 - v1; bn = np.linalg.norm(bisector)
                if bn > 1e-6:
                    bisector /= bn; test_pt = p_curr + bisector * 0.01
            ...
            if e.dxftype() in ('LWPOLYLINE', 'POLYLINE', 'SPLINE', 'CIRCLE'):
                try:
                    if e.dxftype() == 'CIRCLE':
                        ...
                        path_obj = ezdxf.path.make_path(e); pts = list(path_obj.flattening(0.01))
...
def analyze_holes(geometry):
    ...
    def check_poly(p):
        ...
        return w
    for p in polys:
        d = check_poly(p); if d: sizes.append(round(d, 2))
        for interior in p.interiors:
            d = check_poly(Polygon(interior)); if d: sizes.append(round(d, 2))
...
def find_drill_points(geometry, target_dia, tolerance=0.1):
    ...
    def check_poly(p):
        ...
        return p.centroid
    for p in polys:
        pt = check_poly(p); if pt: drill_points.append(pt)
        for interior in p.interiors:
            pt = check_poly(Polygon(interior)); if pt: drill_points.append(pt)
    return drill_points
...
def generate_drill_gcode(points, z_start, z_final, peck_depth, feed, tool_name, header, footer, fmt):
    ...
    for pt in points:
        gc.append(f"; Drill X{pt.x:.3f} Y{pt.y:.3f}"); gc.append(f"{G0} X{pt.x:.3f} Y{pt.y:.3f}"); gc.append(f"{G0} Z{z_start + 1.0}")
        current_z = z_start
        while current_z > z_final:
            target_z = max(current_z - peck_depth, z_final); gc.append(f"{G1} Z{target_z:.3f}")
            if target_z > z_final: gc.append(f"{G0} Z{z_start + 0.5}"); gc.append(f"{G0} Z{target_z + 0.5}")
            current_z = target_z
...
def generate_chamfer_separated(geometry, width, tip_offset, finish_allowance=0.0):
    ...
    try:
        p_finish = geometry.buffer(tip_offset, join_style=1); p_list = ensure_list_of_polys(p_finish)
        for poly in p_list: finish_paths.append(poly.exterior); finish_paths.extend(poly.interiors)
    except: pass
    return ([LineString(ls.coords) for ls in rough_paths], [LineString(ls.coords) for ls in finish_paths])

# --- Vカーブ グラフ理論ロジック (Ver 1.9: ヒゲ除去・角度フィルタ強化) ---
...
class PathGraph:
    ...
    def prune_logic(self, min_len):
        pruned = 0; leaves = [n for n, neigh in self.adj.items() if len(neigh) == 1]
        for leaf in leaves:
            ...
            if dist < min_len and len(self.adj[neighbor]) > 2:
                self.adj[neighbor].remove(leaf); del self.adj[leaf]; pruned += 1
        return pruned

    def prune_short_leaves(self, min_len=3.0):
        for _ in range(15):
            if self.prune_logic(min_len) == 0: break
...
def generate_vcarve(geometry, angle_deg, use_limit, max_d, step_len=0.1):
    ...
    for poly in polys:
        ...
        line = smooth.exterior; length = line.length; sample_res = 0.2
        num = max(50, min(8000, int(length / sample_res))); pts = [line.interpolate(i * length / (num-1)) for i in range(num)]
        ...
        for (p1_idx, p2_idx), (v1_idx, v2_idx) in zip(vor.ridge_points, vor.ridge_vertices):
            ...
            # 隣接点フィルタをさらに厳格化 (サンプリングの4倍未満はノイズ)
            if np.linalg.norm(g1 - g2) < sample_res * 4.0: continue
            graph.add_edge(v1, v2)
    graph.prune_short_leaves(min_len=3.0); raw_chains = graph.get_chains()
...
def make_gcode_phases_advanced(phases, tool_name, header, footer, fmt="G00/G01", is_3d=False):
    ...
    for i, phase in enumerate(phases):
        ...
        feed = int(phase.get('feed', 300)); z_start = phase.get('z_start', 0); z_final = phase.get('z_final', 0); z_step = max(0.01, phase.get('z_step', abs(z_final - z_start))); use_ramp = phase.get('use_ramp', False)
        gc.append(f"; Phase {i+1}: {phase.get('name','')} (F{feed})"); gc.append(f"F{feed}")
        if is_3d:
            for p_pts in paths:
                if not p_pts: continue
                gc.append(f"{G0} X{p_pts[0][0]:.3f} Y{p_pts[0][1]:.3f}"); gc.append(f"{G1} Z{p_pts[0][2]:.3f}")
            ...
            while cur_z > z_final:
                ...
                for path in paths:
                    ...
                    gc.append(f"{G0} X{coords[0,0]:.3f} Y{coords[0,1]:.3f}"); gc.append(f"{G0} Z{cur_z + 1.0}")
                    if use_ramp:
                        dist = 0.0; p_len = path.length; z_d = cur_z - tgt_z; reached = False
                        for j in range(1, len(coords)):
                            dist += np.linalg.norm(coords[j] - coords[j-1]); ratio = min(dist / min(p_len, 50.0), 1.0)
                            gc.append(f"{G1} X{coords[j,0]:.3f} Y{coords[j,1]:.3f} Z{cur_z - (z_d * ratio):.3f}")
                            if ratio >= 1.0:
                                reached = True
                                for k in range(j+1, len(coords)): gc.append(f"{G1} X{coords[k,0]:.3f} Y{coords[k,1]:.3f} Z{tgt_z:.3f}")
                                break
                        if not reached: gc.append(f"{G1} Z{tgt_z:.3f}")
                    else:
...
st.title("⚡ yosikeiCAM 1.9")
st.caption("Ver 1.9: Vカーブ・ディープクリーニング（ヒゲ除去・角度フィルタ強化）")
...
with st.sidebar:
    ...
    with tab1:
        enable_pocket = st.checkbox("有効", True, key="cp"); st.divider()
        dia = st.number_input("工具径 (mm)", value=3.0, min_value=0.01, step=0.1, format="%.3f")
        clear = st.number_input("仕上げ代 (mm)", value=0.0, step=0.1); c1, c2 = st.columns(2)
        ...
        st.caption("▼ 送り速度"); f_r = st.number_input("粗送り速度", value=300, min_value=1, step=50, key="fr")
        f_f = f_r; f_m = "Step-down"
        if clear > 0:
            st.markdown("---"); st.caption("▼ 仕上げ設定"); c3, c4 = st.columns(2)
            with c3: f_f = st.number_input("仕上げ速度", value=300, min_value=1, step=50, key="ff")
            with c4: f_o = st.radio("仕上げ深さ", ["刻み", "一括"], index=0, key="fo"); f_m = "Full-Depth" if f_o == "一括" else "Step-down"
    with tab2:
        enable_chamfer = st.checkbox("有効", True, key="cc"); st.divider()
        cw = st.number_input("幅 (mm)", 0.5, step=0.1); to = st.number_input("刃先オフセット", value=1.0, min_value=0.0, step=0.1); z_c = -(cw + to)
        fc_r = st.number_input("速度", value=300, min_value=1, step=50, key="fcr")
        ucf = st.checkbox("2回加工", False, key="uccf"); cfa = 0.0; fc_f = 300
        if ucf:
            c1, c2 = st.columns(2)
    ...
    with tab3:
        enable_vcarve = st.checkbox("有効", False, key="cv"); st.divider(); va = st.number_input("角度", 60.0, step=10.0); uvl = st.checkbox("深さ制限", False)
        vl = st.number_input("制限 (mm)", value=-3.0) if uvl else -100.0; fv = st.number_input("速度", value=300, min_value=1, step=50, key="fv"); vr = st.slider("精度", 0.2, 0.02, 0.05)
    with tab4:
        enable_drill = st.checkbox("有効", False, key="cd"); st.divider(); ddt = st.number_input("穴径 (mm)", value=3.0, min_value=0.01, step=0.1); dd = st.number_input("深さ Z", value=-5.0); pd = st.number_input("ペック", value=2.0, min_value=0.1); fd = st.number_input("速度", value=200, min_value=1, step=50, key="fd")
    st.divider(); ppn = st.selectbox("ポストプロセッサ", list(POST_PROCESSORS.keys())); pp = POST_PROCESSORS[ppn]
    with st.expander("Gコード詳細"): h_c = st.text_area("Header", pp["start"]); f_c = st.text_area("Footer", pp["end"])

st.header("1. DXFアップロード"); f = st.file_uploader("", type=["dxf"])
if f:
    bn = os.path.splitext(f.name)[0]; polys_raw = dxf_to_shapely_list(f.getvalue())
    if polys_raw:
        tu = unary_union(polys_raw); minx, miny, maxx, maxy = tu.bounds; w, h = maxx-minx, maxy-miny; ox, oy = 0, 0
        ...
        with c1:
            ...
            for i, p in enumerate(polys_moved):
                st_l = 'k-' if i in selected else 'k:'; al = 1.0 if i in selected else 0.1
                ax.plot(*p.exterior.xy, st_l, alpha=al, linewidth=1)
                for interior in p.interiors: ax.plot(*interior.xy, st_l, alpha=al, linewidth=1)
            if enable_drill and gfc:
                dp = find_drill_points(gfc, ddt)
                for pt in dp: ax.plot(pt.x, pt.y, 'x', color='tab:purple')
            ax.axis('equal'); ax.grid(True, linestyle=':'); st.pyplot(fig)
        with c2:
            st.header("2. パス生成"); gc_p, gc_c, gc_v, gc_d = None, None, None, None; dpr, dpf, dpc, dpv = [], [], [], []
            if gfc and not gfc.is_empty:
                if enable_pocket:
                    p_r, p_f = generate_pocket(gfc, dia, clear, step, use_dogbone); dpr, dpf = p_r, p_f; phs = []
                    if p_r: phs.append({'name': 'Rough', 'paths': p_r, 'z_start': 0, 'z_final': depth, 'feed': f_r, 'z_step': z_step_p, 'use_ramp': use_ramp})
                    if p_f: phs.append({'name': 'Finish', 'paths': p_f, 'z_start': 0, 'z_final': depth, 'feed': f_f, 'z_step': (abs(depth) if f_m == "Full-Depth" else z_step_p), 'use_ramp': use_ramp})
                    if phs: gc_p = make_gcode_phases_advanced(phs, "EndMill", h_c, f_c, pp["format"])
                if enable_chamfer:
                    rp, fp = generate_chamfer_separated(gfc, cw, to, cfa if ucf else 0.0); dpc = rp + fp; phs = []
                ...
                if enable_vcarve:
                    with st.spinner("VCarve..."): vps = generate_vcarve(gfc, va, uvl, vl, vr); dpv = vps
                    if vps: gc_v = make_gcode_phases_advanced([{'name': 'V-Carve', 'paths': vps, 'z_start': 0, 'z_final': 0, 'feed': fv}], "VBit", h_c, f_c, pp["format"], True)
                if enable_drill:
                    dpts = find_drill_points(gfc, ddt)
                    if dpts: gc_d = generate_drill_gcode(dpts, 0, dd, pd, fd, f"Drill {ddt}mm", h_c, f_c, pp["format"])
            ...
            for pts in dpv: ax2.plot([p[0] for p in pts], [p[1] for p in pts], color='tab:red', linewidth=0.8)
