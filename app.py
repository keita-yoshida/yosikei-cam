        ...
        for i in range(num_pts):
            ...
            if 5 < abs(angle_deg) < 175:
                ...
                if bn > 1e-6:
                    ...
                    center = p_curr + bisector * ((r / math.sin(max(0.1, math.radians((180-abs(angle_deg))/2)))) - r + (r*0.05))
...
def find_drill_points(geometry, target_dia, tolerance=0.1):
    ...
    for p in polys:
        ...
        for interior in p.interiors:
            pt = check_p(Polygon(interior)); if pt: drill_points.append(pt)
    return drill_points
...
def generate_vcarve(geometry, angle_deg, use_limit, max_d, step_len=0.1):
    ...
    combined_geom = unary_union(polys); boundary = combined_geom.boundary # 距離測定用の「全壁面」
    
    for poly in polys:
        smooth = poly.simplify(0.02, preserve_topology=True)
        line = smooth.exterior; sample_res = 0.2
    ...
    for chain in chains:
        ...
        for i in range(pts_count):
            ...
            d = pt.distance(boundary) # 正しく一番近い壁までの距離を測る
            z = -(d / tan_a)
...
st.title("⚡ yosikeiCAM 2.2")
st.caption("Ver 2.2: 深刻なZ軸突き刺さりバグを修正・最終安定版")
...
with st.sidebar:
    ...
    with tab1:
        enable_pocket = st.checkbox("有効", True, key="cp"); dia = st.number_input("工具径 (mm)", 3.0, step=0.1); clear = st.number_input("代", 0.0, step=0.1)
        with st.container():
            c1, c2 = st.columns(2)
            with c1: depth = st.number_input("深さ Z", -1.0, step=0.1)
            with c2: z_step_p = st.number_input("ピッチ", 1.0, min_value=0.01)
        f_r = st.number_input("送り", 300, key="fr")
    with tab3:
        enable_vcarve = st.checkbox("有効", False, key="cv"); va = st.number_input("角度", 60.0, step=10.0); uvl = st.checkbox("深さ制限", False)
        vl = st.number_input("制限 (mm)", -3.0) if uvl else -100.0; fv = st.number_input("速度", 300, key="fv"); vr = st.slider("精度", 0.2, 0.02, 0.05)
    with tab4:
        enable_drill = st.checkbox("有効", False, key="cd"); ddt = st.number_input("穴径 (mm)", 3.0); dd = st.number_input("深さ", -5.0); pd = st.number_input("ペック", 2.0); fd = st.number_input("送り", 200, key="fd")
...
if f:
    ...
    if polys_raw:
        ...
        cont = st.sidebar.container(); all_c = cont.checkbox("すべて選択", True, key="sa")
        for i, p in enumerate(polys_moved):
            if cont.checkbox(f"Path #{i+1}", all_c, key=f"p{i}"): selected.append(i)
        target_polys = [polys_moved[i] for i in selected]; gfc = merge_polygons_xor(target_polys)
        c1, c2 = st.columns(2)
        with c1:
            st.success(f"サイズ: {w:.1f} x {h:.1f} mm")
        ...
        with c2:
            st.header("2. パス生成"); gc_v = None; dpv = []
            ...
            fig2, ax2 = plt.subplots(figsize=(5,5)); ax2.plot(0, 0, 'r+'); ax2.axhline(0, alpha=0.2); ax2.axvline(0, alpha=0.2)
            for p in polys_moved: ax2.plot(*p.exterior.xy, 'k--', alpha=0.05)
            for pts in dpv: ax2.plot([p[0] for p in pts], [p[1] for p in pts], 'r-', linewidth=0.8)
            ax2.axis('equal'); st.pyplot(fig2)
            if gc_v: st.download_button("📥 VCARVE 保存", gc_v, f"{bn}_vcarve.nc")
