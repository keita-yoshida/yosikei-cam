...
if f:
    ...
    if polys_raw:
        ...
        with c2:
            ...
            vps_disp, p_r_disp, p_f_disp, c_disp = [], [], [], []
            ...
            if gfc and not gfc.is_empty:
                if enable_v:
                    with st.spinner("VCarve..."):
                        ...
                        vps_disp = vps
                    if vps:
                        gc_v = make_gcode_phases_advanced([{'name': 'V-Carve', 'paths': vps, 'z_start': 0, 'z_final': 0, 'feed': fed_v}], "VBit", h_c, f_c, pp["format"], True)
                
                if enable_p:
                    p_r, p_f = generate_pocket(gfc, dia, clear, 0.5, True)
                    p_r_disp, p_f_disp = p_r, p_f
                    phs = []
                    if p_r: phs.append({'name': 'Rough', 'paths': p_r, 'z_start': 0, 'z_final': dep_p, 'feed': fed_p, 'z_step': stp_p})
                    if p_f: phs.append({'name': 'Finish', 'paths': p_f, 'z_start': 0, 'z_final': dep_p, 'feed': fed_p, 'z_step': dep_p})
            ...
            for ls in p_r_disp: ax2.plot(*ls.xy, color='tab:blue', alpha=0.3)
            for ls in p_f_disp: ax2.plot(*ls.xy, color='tab:cyan', alpha=0.8)
            for ls in c_disp: ax2.plot(*ls.xy, color='tab:green', alpha=0.8)
            for pts in vps_disp: ax2.plot([p[0] for p in pts], [p[1] for p in pts], 'r-', linewidth=0.8)
