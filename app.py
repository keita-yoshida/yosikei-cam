...
def apply_dogbone_single(polygon: Polygon, tool_dia: float) -> Polygon:
    ...
    for ring_idx, ring in enumerate(rings):
        ...
        for i in range(n):
            ...
            v_in = p_curr - p_prev
            v_out = p_next - p_curr
            mag_in, mag_out = np.linalg.norm(v_in), np.linalg.norm(v_out)
...
def find_drill_points(geometry, target_dia, tolerance=0.1):
    ...
    for p in polys:
        ...
        for interior in p.interiors:
            ...
            if pt_h: drill_points.append(pt_h)
    return drill_points
...
def generate_chamfer_separated(geometry, width, tip_offset, finish_allowance=0.0):
    ...
    if finish_allowance > 0:
        try:
            p_r = geometry.buffer(tip_offset + finish_allowance, join_style=1); p_list = ensure_list_of_polys(p_r)
            for poly in p_list: rough_paths.append(poly.exterior); rough_paths.extend(poly.interiors)
        except: pass
    try:
        p_f = geometry.buffer(tip_offset, join_style=1); p_list_f = ensure_list_of_polys(p_f)
        for poly in p_list_f: finish_paths.append(poly.exterior); finish_paths.extend(poly.interiors)
    except: pass
...
with st.sidebar:
    ...
    with tab1:
        enable_p = st.checkbox("有効", value=True, key="cp"); dia = st.number_input("工具径 (mm)", value=3.0, step=0.1)
        ucp = st.checkbox("2回加工 (粗+仕上げ)", value=False, key="uccp"); clear = 0.0; f_p_r = st.number_input("送り速度 (粗)", value=300, min_value=1, key="fp_r")
        f_p_f = f_p_r; f_mode = "Step-down"
        if ucp:
            clear = st.number_input("仕上げ代 (mm)", value=0.2, step=0.1)
            c3, c4 = st.columns(2)
            with c3: f_p_f = st.number_input("仕上げ送り", value=300, min_value=1, key="fp_f")
            with c4: f_o = st.radio("仕上げ深さ", ["ピッチ刻み", "最終一括"], index=0, key="fmp"); f_mode = "Full-Depth" if f_o == "最終一括" else "Step-down"
        c1, c2 = st.columns(2)
    ...
    with tab2:
        enable_c = st.checkbox("有効", value=True, key="cc"); cw = st.number_input("幅 (mm)", value=0.5, step=0.1); to = st.number_input("刃先オフセット", value=1.0, step=0.1); z_c = -(cw + to)
        fc_r = st.number_input("速度", value=300, min_value=1, key="fcr"); ucf = st.checkbox("2回加工", value=False, key="uccf"); cfa = 0.0; fc_f = fc_r
        if ucf:
            c1, c2 = st.columns(2)
            with c1: cfa = st.number_input("代 (mm)", value=0.2, min_value=0.01, step=0.1)
            with c2: fc_f = st.number_input("仕上送り", value=300, min_value=1, key="fcf")
    with tab3:
        enable_v = st.checkbox("有効", value=False, key="cv"); va = st.number_input("角度", value=60.0, step=10.0); ucv = st.checkbox("2回加工", value=False, key="uccv")
        v_cl = 0.0; fvf = 300; fv = st.number_input("送り速度 (粗)", value=300, key="fv")
        if ucv:
            v_cl = st.number_input("仕上げ代 (mm)", value=0.2, step=0.1); fvf = st.number_input("仕上げ送り", value=300, key="fvf")
        uvl = st.checkbox("深さ制限", value=False); vl = st.number_input("最大深さ", value=-3.0) if uvl else -100.0; vr = st.slider("計算精度", value=0.05, min_value=0.02, max_value=0.2)
    with tab4:
        enable_d = st.checkbox("有効", value=False, key="cd"); ddt = st.number_input("穴径 (mm)", value=3.0); ddz = st.number_input("深さ", value=-5.0); pck = st.number_input("ペック", value=2.0, min_value=0.1); fd = st.number_input("送り", value=200, key="fd")
    st.divider(); ppn = st.selectbox("ポストプロセッサ", list(POST_PROCESSORS.keys())); pp = POST_PROCESSORS[ppn]; h_c = st.text_area("Header", pp["start"]); f_c = st.text_area("Footer", pp["end"])
...
if f:
    ...
    if polys_raw:
        ...
        st.sidebar.divider(); st.sidebar.subheader("📐 パス選択"); cont = st.sidebar.container(); all_c = cont.checkbox("すべて選択", value=True, key="sa"); selected = []
        for i, p in enumerate(polys_moved):
            if cont.checkbox(f"Path #{i+1} (Area:{p.area:.1f})", value=all_c, key=f"p{i}"): selected.append(i)
        gfc = merge_polygons_xor([polys_moved[i] for i in selected])
        if gfc:
            hs = analyze_holes(gfc)
            if hs: st.sidebar.info("💡 穴: " + ", ".join([f"φ{d}({c}個)" for d, c in hs.items()]))
        c1, c2 = st.columns(2)
        with c1:
            st.success(f"サイズ: {maxx-minx:.1f} x {maxy-miny:.1f} mm")
        ...
        with c2:
            ...
            if gfc and not gfc.is_empty:
                ...
                if enable_c:
                    ...
                    if rp and ucf: phs.append({'name':'Rough','paths':rp,'z_start':0,'z_final':z_c,'feed':fc_r})
